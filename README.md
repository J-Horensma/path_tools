...
#### filter_path(PATH)
### **Description:**
**1.)** Uses path_tools' built-in "appropriate_quotes()" function, to remove any prepended quotes and matching quotes, at the end of the supplied path (If present),\ 
then replace any OS-inappropriate quotes, in the path, with the OS-appropriate quotes, then the "appropriate_quotes()" function,\ 
